...
public:
	...
	int numOfConnection(Vertex v) const;
...
template <class Vertex>
int UListGraph<Vertex>::numOfConnection(Vertex v) const{
	if (!containsVertex(v)) {
		throw NoSuchElement();
	}
	std::set<Vertex> incidents = this->getConnectionFrom(v);
	return incidents.size();
}

template <class Vertex>
std::string UListGraph<Vertex>::getConnections() const {
	std::stringstream aux;
	typename std::set<Vertex>::iterator i;

	for (i = vertexes.begin(); i != vertexes.end(); i++) {
		aux << "The vertex " << (*i);
		aux << " has a total of "<< numOfConnection(*i) << " connections." <<std::endl;
	}
...
template <class Vertex>
std::string UListGraph<Vertex>::getMaxConnections() const {

	std::stringstream aux;
	typename std::set<Vertex>::const_iterator i;
	int j;
	int max = 0;

	for (i = vertexes.begin(); i != vertexes.end(); i++) {
		if(numOfConnection(*i) > max){
			max = numOfConnection(*i);
			j = *i;
		}
	}

	aux << "Vertex " << j;
	aux << " has the highest count, with a total of "<< max << "\n\n";
	return aux.str();
}
